    ...
    @if ($LatestPosts)
        @foreach ($LatestPosts as $LatestPost)
            <div class="slider-item" style="background-image:url({{url('uploads/Posts/' . $LatestPost->image)}});" data-stellar-background-ratio="0.5">
                {{-- <img src="{{url('uploads/Posts/' . $LatestPost->image)}}" alt=""> --}}
                ...
                <div class="container">
                    ...
                    <div class="col-md-6 text ftco-animate">
                        ...
                        <p><a href="{{route('front.post' , [$LatestPost->id , str_replace_me($LatestPost->name)])}}" title="{{$LatestPost->name}}" class="btn btn-secondary px-4 py-3 mt-3">@lang('lang.Show Details')</a></p>
                    </div>
                    </div>
                </div>
            </div>
